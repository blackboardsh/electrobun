...
if (platform === "win32" && process.env.GITHUB_ACTIONS) {
  ...
  execSync(
    `set "BUN_INSTALL_CACHE_DIR=${bunCacheDir}" && bun build src/cli/index.ts --compile ${compileTarget} --outfile bin/electrobun`,
    { stdio: "inherit", shell: true },
  );
} else {
  execSync(
    `bun build src/cli/index.ts --compile ${compileTarget} --outfile bin/electrobun`,
    { stdio: "inherit" },
  );
}
