...
const temporaryDirectory = mkdtempSync(
	join(tmpdir(), "electrobun-linux-x11-geometry-"),
);
const binary = join(
	temporaryDirectory,
	`linux-x11-geometry-test${executableSuffix}`,
);
...
try {
	...
	if (compile.status !== 0) {
		throw new Error(
			`Linux X11 geometry native test compilation exited with ${compile.status ?? 1}`,
		);
	}
	...
	if (test.status !== 0) {
		throw new Error(
			`Linux X11 geometry native test exited with ${test.status ?? 1}`,
		);
	}
} finally {
