...
async function vendorAsar() {
  ...
  for (const arch of archsToDownload) {
    ...
    try {
      ...
      console.error(
        `Failed to download zig-asar binaries for ${arch}:`,
        error.message,
      );
      throw new Error(
  ...
  if (OS === "macos") {
    ...
    try {
      ...
      try {
        ...
        const cflagsResult =
          await $`pkg-config --cflags webkit2gtk-4.1 gtk+-3.0 ayatana-appindicator3`.quiet();
        pkgConfigCflags = cflagsResult.stdout.toString().trim();
        const libsResult =
          await $`pkg-config --libs webkit2gtk-4.1 gtk+-3.0 ayatana-appindicator3`.quiet();
