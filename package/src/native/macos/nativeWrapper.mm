...
// typedef BOOL (*DecideNavigationCallback)(uint32_t webviewId, const char*
// url); NOTE: Bun's FFIType.true doesn't play well with objective C's YES/NO
// char booleans so when sending booleans from JSCallbacks we have to use u32
// for now
typedef uint32_t (*DecideNavigationCallback)(uint32_t webviewId,
                                             const char *url);
typedef void (*WebviewEventHandler)(uint32_t webviewId, const char *type,
                                    const char *url);
typedef BOOL (*HandlePostMessage)(uint32_t webviewId, const char *message);
typedef const char *(*HandlePostMessageWithReply)(uint32_t webviewId,
                                                  const char *message);
...
static std::string getMimeTypeFromUrl(const std::string &url) {
  ...
  if (url.find(".html") != std::string::npos ||
    ...
    return "text/html";
             ...
             url.find(".mjs") != std::string::npos ||
             url.find(".cjs") != std::string::npos) {
    return "text/javascript";
  } else if (url.find(".ts") != std::string::npos ||
             url.find(".mts") != std::string::npos ||
             url.find(".cts") != std::string::npos) {
  ...
  } else if (url.find(".yaml") != std::string::npos ||
             url.find(".yml") != std::string::npos) {
...
// typedef void (*WindowResizeHandler)(uint32_t windowId, CGFloat x, CGFloat y,
// CGFloat width, CGFloat height);
typedef void (*WindowCloseHandler)(uint32_t windowId);
typedef void (*WindowMoveHandler)(uint32_t windowId, double x, double y);
typedef void (*WindowResizeHandler)(uint32_t windowId, double x, double y,
...
/** Snapshot callback. */
typedef void (*zigSnapshotCallback)(uint32_t hostId, uint32_t webviewId,
                                    const char *dataUrl);
...
enum class PermissionType { USER_MEDIA, GEOLOCATION, NOTIFICATIONS, OTHER };
...
static std::map<std::pair<std::string, PermissionType>, PermissionCacheEntry>
    g_permissionCache;
...
PermissionStatus getPermissionFromCache(const std::string &origin,
                                        PermissionType type) {
...
void cachePermission(const std::string &origin, PermissionType type,
                     PermissionStatus status) {
  auto key = std::make_pair(origin, type);
...
NSData *readViewsFile(const char *viewsUrl) {
  ...
  NSString *relativePath =
      [urlString substringFromIndex:8]; // "views://" is 8 chars
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:asarPath]) {
    ...
    if (g_asarArchive) {
      ...
      if (fileData && fileSize > 0) {
        NSLog(@"DEBUG readViewsFile: Read %zu bytes from ASAR for %@", fileSize,
              relativePath);
  ...
  NSLog(@"DEBUG readViewsFile: file exists=%@",
        [[NSFileManager defaultManager] fileExistsAtPath:filePath] ? @"YES"
                                                                   : @"NO");
...
- (void)resize:(NSRect)frame withMasksJSON:(const char *)masksJson {
  ...
  CAShapeLayer * (^createMaskLayer)(void) = ^CAShapeLayer * {
    ...
    NSError *error = nil;
    NSArray *rectsArray = [NSJSONSerialization JSONObjectWithData:jsonData
                                                          options:0
                                                            error:&error];
    if (!rectsArray || error) {
      return nil;
    ...
    NSArray<NSValue *> *processedRects =
        addOverlapRects(rectsArray, heightToAdjust);
...
- (void)webView:(WKWebView *)webView
  ...
  if ([urlString hasPrefix:@"views://"]) {
    ...
    if ([relativePath isEqualToString:@"/internal/index.html"]) {
      ...
      NSLog(@"DEBUG: Handling views://internal/index.html for webview %u",
            self.webviewId);
      ...
      if (!contentPtr) {
        ...
        NSLog(@"DEBUG: No HTML content found for webview %u, using fallback",
              self.webviewId);
      ...
      if (contentPtr) {
        ...
        NSLog(@"DEBUG WKWebView: HTML content length: %zu, content preview: "
              @"%.100s",
              contentLength, contentPtr);
      ...
      if (data) {
        NSLog(@"DEBUG WKWebView: Successfully read views file, length: %lu",
  ...
  if (contentPtr && contentLength > 0) {
    ...
    NSLog(@"DEBUG WKWebView: Set MIME type '%s' for URL: %@",
          detectedMimeType.c_str(), urlString);
    ...
    NSLog(@"DEBUG WKWebView: Sending response with MIME type: %@, encoding: %@",
          mimeType, encodingName);
    ...
    NSLog(@"============== ERROR ========== empty response for URL: %@",
          urlString);
...
- (void)webView:(WKWebView *)webView
  ...
  NSLog(@"DEBUG WKWebView Navigation: webview %u navigating to %@",
        self.webviewId, newURL.absoluteString);
...
- (void)userContentController:(WKUserContentController *)userContentController
  ...
  // Note: threadsafe JSCallbacks are invoked on the js worker thread, When
  // called frequently they can build up and take longer. Meanwhile objc GC auto
  // free's the message body and the callback ends up getting garbage.

  // So we duplicate it and give it plenty of time to execute (1 second delay
  // vs. 0.1ms execution per invocation) before freeing the memory
  dispatch_after(
...
- (instancetype)initWithWebviewId:(uint32_t)webviewId
  ...
  if (self) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      self.webView.frame = NSMakeRect(frame.origin.x, adjustedY,
      ...
      NSLog(@"DEBUG WKWebView Constructor: URL check - url=%p, url='%s', "
            @"strlen=%zu",
            url, url ? url : "NULL", url ? strlen(url) : 0);
      if (url && strlen(url) > 0) {
        NSLog(@"DEBUG WKWebView Constructor: Loading initial URL: %s", url);
        [self loadURL:url];
      } else {
        NSLog(@"DEBUG WKWebView Constructor: Skipping URL load - no URL or "
              @"empty URL");
      }
...
- (void)remove {
  if (self.webView) {
    ...
    @try {
      ...
      NSLog(@"WKWebViewImpl remove: Exception removing KVO observer for "
            @"webview %u: %@",
            self.webviewId, exception);
    }

    // Remove from ContainerView's tracking array
    if (self.webView.superview &&
        [self.webView.superview isKindOfClass:[ContainerView class]]) {
    ...
    // Set delegates to nil to prevent callbacks during cleanup
    self.webView.navigationDelegate = nil;
    self.webView.UIDelegate = nil;

    NSLog(@"WKWebViewImpl remove: evaluating cleanup JavaScript for webview %u",
          self.webviewId);

    // Remove from superview SYNCHRONOUSLY before releasing
    // This prevents the dangling pointer issue in the async block
    if (self.webView.superview != nil) {
      NSLog(@"WKWebViewImpl remove: removing from superview for webview %u",
            webviewIdForLogging);
      [self.webView removeFromSuperview];
      NSLog(@"WKWebViewImpl remove: removed from superview for webview %u",
            webviewIdForLogging);
    }

    NSLog(@"WKWebViewImpl remove: releasing webView object for webview %u",
          self.webviewId);
    releaseObjCObject(self.webView);
    self.webView = nil;
    self.nsView = nil;
    NSLog(@"WKWebViewImpl remove: webView set to nil for webview %u",
          webviewIdForLogging);
  } else {
    NSLog(@"WKWebViewImpl remove: webView is already nil for webview %u",
          self.webviewId);
  }

  NSLog(@"WKWebViewImpl remove: COMPLETED cleanup for webview %u",
        self.webviewId);
}
...
- (void)dealloc {
  // The observer may have already been removed in -remove method
  // Only try to remove if webView is still valid
  if (self.webView) {
    @try {
      [self.webView removeObserver:self forKeyPath:@"fullscreenState"];
    } @catch (NSException *exception) {
      // Observer was already removed or never added - this is fine
    }
  }
}
...
public:
  ...
  void OnBeforeCommandLineProcessing(
    ...
    // Note: This stops CEF (Chromium) trying to access Chromium's storage for
    // system-level things like credential management. Using a mock keychain
    // just means it doesn't use keychain for credential storage. Other security
    // features like cookies, https, etc. are unaffected.
...
public:
  ElectrobunClient(uint32_t webviewId, HandlePostMessage bunBridgeHandler,
                   HandlePostMessage internalBridgeHandler,
                   WebviewEventHandler webviewEventHandler,
                   DecideNavigationCallback navigationCallback)
      : webview_id_(webviewId), bun_bridge_handler_(bunBridgeHandler),
  ...
  OnProcessMessageReceived(CefRefPtr<CefBrowser> browser,
    ...
    // Note: threadsafe JSCallbacks are invoked on the js worker thread, When
    // called frequently they can build up and take longer. Meanwhile objc GC
    // auto free's the message body and the callback ends up getting garbage.
  ...
  bool OnContextMenuCommand(CefRefPtr<CefBrowser> browser,
    ...
    if (command_id == MENU_ID_DEV_TOOLS) {
      ...
      browser->GetHost()->ShowDevTools(
          windowInfo, browser->GetHost()->GetClient(), settings, inspect_at);
      return true;
    }
    return false;
  }
  ...
  virtual bool OnShowPermissionPrompt(
    ...
    NSString *message = @"This page is requesting additional "
                        @"permissions.\n\nDo you want to allow this?";
...
bool initializeCEF() {
  ...
  // pathForAuxiliaryExecutable:@"bun Helper.app/Contents/MacOS/bun Helper"]; if
  // (helperPath) {
  ...
  std::string appIdentifier =
      !g_electrobunIdentifier.empty() ? g_electrobunIdentifier : "Electrobun";
...
CreateRequestContextForPartition(const char *partitionIdentifier,
                                 uint32_t webviewId) {
  NSLog(@"DEBUG CEF: CreateRequestContextForPartition called for webview %u, "
        @"partition: %s",
  ...
  if (!partitionIdentifier || !partitionIdentifier[0]) {
    ...
    if (isPersistent) {
      ...
      std::string appIdentifier = !g_electrobunIdentifier.empty()
      ...
      NSString *cachePath = [[[[appSupportPath
          ...
          stringByAppendingPathComponent:[NSString
                                             stringWithUTF8String:partitionName
                                                                      .c_str()]]
          copy];
      NSFileManager *fileManager = [NSFileManager defaultManager];
  ...
  bool registered =
      context->RegisterSchemeHandlerFactory("views", "", schemeFactory);
  NSLog(@"DEBUG CEF: Registered scheme handler factory for partition '%s' - "
        @"success: %s",
  ...
  if (self) {
    ...
    void (^createCEFBrowser)(void) = ^{
      ...
      self.client = new ElectrobunClient(
          webviewId, bunBridgeHandler, internalBridgeHandler,
          webviewEventHandler, navigationCallback);
    ...
    // TODO: revisit bug with 3 windows where 2nd windows' oopifs don't get
    // created until moving the mouse and where createCEFBrowser() after async
    // causes a crash
...
extern "C" AbstractView *
initWebview(uint32_t webviewId, NSWindow *window, const char *renderer,
            const char *url, double x, double y, double width, double height,
            ...
            const char *customPreloadScript, bool transparent) {
...
extern "C" void webviewRemove(AbstractView *abstractView) {
  ...
  NSLog(@"webviewRemove: webviewId=%u, globalAbstractViews=%p, count=%lu",
        abstractView.webviewId, globalAbstractViews,
        globalAbstractViews ? (unsigned long)globalAbstractViews.count : 0);
  ...
  if (!trackedView) {
    NSLog(@"webviewRemove: webview %u not found in global tracking, already "
          @"removed - EXITING",
          abstractView.webviewId);
    return;
  }
  ...
  NSLog(@"webviewRemove: Removed webview %u from global tracking (remaining: "
        @"%lu)",
        abstractView.webviewId, (unsigned long)globalAbstractViews.count);

  NSLog(@"webviewRemove: About to call [abstractView remove] for webview %u",
        abstractView.webviewId);
  [abstractView remove];
...
NSWindow *createNSWindowWithFrameAndStyle(
  ...
  NSWindow *window =
      [[NSWindow alloc] initWithContentRect:config.frame
...
extern "C" NSWindow *createWindowWithFrameAndStyleFromWorker(
  ...
  createNSWindowWithFrameAndStyleParams config = {
      .frame = frame, .styleMask = styleMask, .titleBarStyle = titleBarStyle};

  // Use a dispatch semaphore to wait for the window creation to complete
  __block NSWindow *window = nil;
  dispatch_sync(dispatch_get_main_queue(), ^{
    window = createNSWindowWithFrameAndStyle(windowId, config, zigCloseHandler,
                                             zigMoveHandler, zigResizeHandler,
                                             zigFocusHandler);
...
static NSEventModifierFlags parseModifiers(NSString *accelerator,
  ...
  NSMutableArray *parts =
      [[accelerator componentsSeparatedByString:@"+"] mutableCopy];
          ...
          if (filterUrl) {
            ...
            if ([cookieDomain hasPrefix:@"."]) {
              matches =
                  ...
                  [host isEqualToString:[cookieDomain substringFromIndex:1]];
            } else {
            ...
            if ([filterDomain hasPrefix:@"."]) {
              ...
              matches = [cookieDomain isEqualToString:filterDomain] ||
                        [cookieDomain
                            isEqualToString:
                                [@"." stringByAppendingString:filterDomain]];
            }
          }
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    @autoreleasepool {
      ...
      if (typesStr.length > 0) {
        ...
        // Clear all
        dataTypes =
            [NSMutableSet setWithSet:[WKWebsiteDataStore allWebsiteDataTypes]];
      }
