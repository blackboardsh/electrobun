...
public:
    ...
    ContainerView(GtkWidget* window) : window(window), windowId(0), closeCallback(nullptr), moveCallback(nullptr), resizeCallback(nullptr), focusCallback(nullptr), blurCallback(nullptr) {
    ...
    ContainerView(GtkWidget* window, uint32_t windowId, WindowCloseCallback closeCallback, WindowMoveCallback moveCallback, WindowResizeCallback resizeCallback, WindowFocusCallback focusCallback, WindowFocusCallback blurCallback)
        : window(window), windowId(windowId), closeCallback(closeCallback), moveCallback(moveCallback), resizeCallback(resizeCallback), focusCallback(focusCallback), blurCallback(blurCallback) {
        // Create an overlay container as the main container
        overlay = gtk_overlay_new();
        gtk_container_add(GTK_CONTAINER(window), overlay);
...
void* createX11Window(uint32_t windowId, double x, double y, double width, double height, const char* title,
                   WindowCloseCallback closeCallback, WindowMoveCallback moveCallback, WindowResizeCallback resizeCallback, WindowFocusCallback focusCallback, WindowBlurCallback blurCallback,
...
ELECTROBUN_EXPORT void* createGTKWindow(uint32_t windowId, double x, double y, double width, double height, const char* title,
    ...
    void* result = dispatch_sync_main([&]() -> void* {
        ...
        auto container = std::make_shared<ContainerView>(window, windowId, closeCallback, moveCallback, resizeCallback, focusCallback, blurCallback);
        ...
        g_signal_connect(window, "focus-out-event", G_CALLBACK(+[](GtkWidget* widget, GdkEventFocus* event, gpointer user_data) -> gboolean {
            ContainerView* container = static_cast<ContainerView*>(user_data);
            if (container && container->blueCallback) {
                container->blueCallback(container->windowId);
            }
            return FALSE; // Allow event to propagate
        }), container.get());
...
ELECTROBUN_EXPORT void* createWindowWithFrameAndStyleFromWorker(uint32_t windowId, double x, double y, double width, double height,
    ...
    if (isCEFAvailable()) {
        return createX11Window(windowId, x, y, width, height, "Window", closeCallback, moveCallback, resizeCallback, focusCallback, blurCallback, titleBarStyle, transparent);
    } else {
        // Pass titleBarStyle and transparent to GTK window creation
        return createGTKWindow(windowId, x, y, width, height, "Window", closeCallback, moveCallback, resizeCallback, focusCallback, blurCallback, titleBarStyle, transparent);
    }
