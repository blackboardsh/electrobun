...
async function main() {
    ...
    if (targetsArg) {
      if (targetsArg === "current") {
        ...
        return targetsArg.split(",").map((target) => {
          ...
          if (
            !["macos", "win", "linux"].includes(os) ||
            !["arm64", "x64"].includes(arch)
          ) {
            console.error(
              `Invalid target: ${target}. Format should be: os-arch (e.g., macos-arm64)`,
            );
            process.exit(1);
          }
  ...
  function parseConfigTargets(): BuildTarget[] {
    ...
    if (config.build.targets && config.build.targets.length > 0) {
      return config.build.targets.map((target) => {
        ...
        if (
          ...
          console.error(
            `Invalid target in config: ${target}. Format should be: os-arch (e.g., macos-arm64)`,
          );
          process.exit(1);
        }
        return { os, arch } as BuildTarget;
    ...
    if (targetsArg === "all") {
      console.log(
        "No targets specified in config, using all available platforms",
      );
      return [
        { os: "macos", arch: "arm64" },
        { os: "macos", arch: "x64" },
  ...
  if (buildTargets.length === 1) {
    console.log(
      `Building for ${buildTargets[0].os}-${buildTargets[0].arch} (${buildEnvironment})`,
    );
  } else {
    const targetList = buildTargets.map((t) => `${t.os}-${t.arch}`).join(", ");
    console.log(
      `Building for multiple targets: ${targetList} (${buildEnvironment})`,
    );
    ...
    for (const result of results) {
      if (result.status === "fulfilled") {
        ...
        console.log(
          `${target.os}-${target.arch}: ${status} (exit code: ${exitCode})`,
        );
  ...
  const bundleFileName =
    targetOS === "macos" ? `${appFileName}.app` : appFileName;
...
<dict>
    ...
    if (config.scripts.postBuild) {
      ...
      const result = Bun.spawnSync(
        [hostPaths.BUN_BINARY, config.scripts.postBuild],
        {
          stdio: ["ignore", "inherit", "inherit"],
      ...
      if (result.exitCode !== 0) {
        console.error(
          "postBuild script failed with exit code:",
          result.exitCode,
        );
        if (result.stderr) {
...
`;
          ...
          const wrappedRunPath = await wrapInArchive(
            selfExtractingLinuxPath,
            buildFolder,
            "tar.gz",
          );
          ...
          execSync(
            `tar -czf ${escapePathForTerminal(linuxTarPath)} -C ${escapePathForTerminal(buildFolder)} ${escapePathForTerminal(basename(appBundleFolderPath))}`,
          );
      ...
      if (!config.release.bucketUrl || config.release.bucketUrl.trim() === "") {
        ...
        console.log(
          "To enable patch generation, configure bucketUrl in your electrobun.config",
        );
      } else {
        ...
        if (updateJsonResponse && updateJsonResponse.ok) {
          ...
          if (response.ok && response.body) {
            ...
            if (!result.success) {
              throw new Error(
                `bsdiff failed with exit code ${result.exitCode}`,
              );
            }
          }
        } else {
...
`;
  ...
  async function createLinuxSelfExtractingBinary(
    ...
    console.log(
      `Created self-extracting Linux binary: ${outputPath} (${(combinedBuffer.length / 1024 / 1024).toFixed(2)} MB)`,
    );
  ...
  async function createAppImage(
    ...
    try {
      ...
      cpSync(appBundlePath, appDirAppPath, {
        recursive: true,
        dereference: true,
      });
...
`;
      ...
      try {
        ...
        execSync(
          `appimagetool ${escapePathForTerminal(appDirPath)} ${escapePathForTerminal(appImagePath)}`,
          { stdio: "inherit" },
        );
        return appImagePath;
      } catch {
        ...
        try {
          ...
          execSync(
            `docker run --rm -v "${buildFolder}:/workspace" linuxserver/appimagetool "/workspace/${basename(appDirPath)}" "/workspace/${basename(appImagePath)}"`,
            { stdio: "inherit" },
          );
          return appImagePath;
        } catch {
          console.warn(
            "Neither appimagetool nor Docker found. AppImage creation skipped.",
          );
