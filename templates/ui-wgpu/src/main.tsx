...
// ---------------------------------------------------------------------------
// Components: plain functions returning JSX
// ---------------------------------------------------------------------------
...
function Button(props: {
	...
	return (
		...
		>
			<text size={14} color={theme.textPrimary}>
				{props.label}
			</text>
		</box>
	);
...
function Divider(): UIElement {
	return <box height={1} bg={theme.line} />;
}

function Header(props: { sizeLabel: Reactive<string> }): UIElement {
	...
	return (
		<row pad={14} align="center" gap={8}>
			...
			<spacer />
			<text size={11} color={theme.textFaint}>
				{props.sizeLabel as never}
			</text>
		</row>
	);
}

function Counter(): UIElement {
	return (
		<column grow={1} justify="center" align="center" gap={16}>
			...
			<row gap={10}>
				...
				<Button label="+ 1" onClick={() => adjust(1)} width={92} />
				<Button label="Reset" onClick={reset} width={92} />
			</row>
			<text size={11} color={theme.textFaint}>
				Keyboard: space or +/- adjust, r resets
			</text>
		</column>
	);
}

function EventLog(): UIElement {
	return (
...
// A native Dawn surface running an animated Mandelbrot zoom; the palette
// phase follows the counter. Same pipeline as the builder edition.
...
function MandelbrotSurface(): UIElement {
	return (
		<column pad={12} gap={6} grow={1}>
			<text size={11} color={theme.textMuted}>
				wgpuSurface - Mandelbrot (palette follows count)
			</text>
			{() => {
				wgpuSurface({
					grow: 1,
...
function WebPanel(): UIElement {
	return (
		<column pad={12} gap={6} grow={1}>
			<text size={11} color={theme.textMuted}>
				webview - https://blackboard.sh
			</text>
			{() => {
				webview({ grow: 1, url: "https://blackboard.sh" });
			}}
		</column>
	);
}
