...
fn fs_main(
  ...
  return vec4<f32>(col, alpha);
	...
	defineTest({
		name: "Transparent Three.js WGPU",
		category: "WGPUView (Interactive)",
		description: "Three.js rendering in a transparent window (DComp compositing test)",
		interactive: true,
		timeout: 120000,
		async run({ log, showInstructions }) {
			await showInstructions([
				"A transparent GPU window will open with a 3D cube",
				"Click and drag to fling the cube",
				"Desktop should be visible behind the scene",
				"Close the window when done",
			]);

			log("Opening transparent Three.js window");

			await new Promise<void>((resolve, reject) => {
				const win = new GpuWindow({
					title: "Transparent Three.js WGPU",
					frame: { width: 600, height: 450, x: 260, y: 180 },
					titleBarStyle: "hiddenInset",
					transparent: true,
				});
				...
				const start = async () => {
					...
					const adapterDevice = WGPUBridge.createAdapterDevice(
						instance as number,
						surface as number,
					);
					if (!adapterDevice) {
						log("Failed to create WGPU adapter/device");
						return;
					}
					const { adapter, device, queue } = adapterDevice;

					WGPUBridge.configureSurface(
						surface as number,
						device as number,
						win.wgpuView.frame.width,
						win.wgpuView.frame.height,
					);

					const renderer = three.createRenderer({
						instance: instance as number,
						adapter: adapter as number,
						device: device as number,
						queue: queue as number,
						surface: surface as number,
						width: win.wgpuView.frame.width,
						height: win.wgpuView.frame.height,
						clearColor: { r: 0, g: 0, b: 0, a: 0 },
					});

					const scene = three.createScene();
					const camera = three.createPerspectiveCamera(
						75,
						win.wgpuView.frame.width / win.wgpuView.frame.height,
						0.1,
						1000,
					);
					three.setCameraPosition(camera, 0, 0, 3);

					const geometry = three.createBoxGeometry(1, 1, 1);
					const material = three.createMeshStandardMaterial({
						color: 0x44aa88,
						roughness: 0.3,
						metalness: 0.7,
					});
					const mesh = three.createMesh(geometry, material);
					three.addToScene(scene, mesh);

					const light = three.createDirectionalLight(0xffffff, 1.5);
					three.setLightPosition(light, 2, 3, 4);
					three.addToScene(scene, light);

					const ambientLight = three.createAmbientLight(0x404040, 0.5);
					three.addToScene(scene, ambientLight);

					let angle = 0;
					const animate = () => {
						angle += 0.02;
						three.setRotation(mesh, angle, angle * 0.7, 0);
						three.render(renderer, scene, camera);
						requestAnimationFrame(animate);
					};
					animate();
					log("Transparent Three.js rendering started");
				};
				start().catch((err) => log(`Error: ${String(err)}`));
