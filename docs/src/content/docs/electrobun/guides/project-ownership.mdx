...
| Concern | Owner |
| --- | --- |
| Electrobun release | Optional exact `hutch.config.ts` → `electrobun.version`; otherwise npm or channel default |
...
`electrobun.version` may be omitted. npm-launched commands then use the exact
Electrobun version paired with the installed `electrobun` dependency; direct
Hutch commands float on the active release channel, while reusing the projected
version between explicit syncs. When present, `electrobun.version` must be an
exact release and overrides either default.
