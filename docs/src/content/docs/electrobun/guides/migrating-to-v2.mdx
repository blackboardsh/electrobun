...
little or no change, and the npm package remains the front door: it now
vendors the Hutch toolchain for your platform, so installing it fully
provisions a project with no global installer, no shell-profile changes, and
...
1. **Bump the dependency.** The 2.x package carries the `electrobun`
   command plus the Hutch launcher for your platform
   (`@electrobun/hutch-<os>-<arch>` optional dependencies). Keep it in
   `devDependencies`; its version now *selects your toolchain* — the paired
   Hutch, Cottontail, and Electrobun versions ride your lockfile, and
   updating the dependency updates all of them together.
