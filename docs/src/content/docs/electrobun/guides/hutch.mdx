...
templates. Templates carry no version pins: npm-launched commands use the
installed package's exact paired defaults, while direct Hutch commands float on
the active release channel until you add a pin.

<Aside type="caution" title="Upgrade global Hutch before using current Vite templates">
The current floating Vite templates require Hutch 0.24 or later for built-in
...
After extracting a template, `init` syncs the resolved Electrobun release — the
npm-paired default when supplied, otherwise the selected catalog release — and
the required native toolchain. It then runs the template's configured `install`
task when one exists. Pass `--skip-install` to defer that task. Initialization
always requires network access because Hutch fetches the current template
catalog and chosen template instead of keeping persistent copies of either one.
...
sync to its resolved npm-paired or template-catalog release.
