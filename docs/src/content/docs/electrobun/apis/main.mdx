...
```rust
// src/rust/main.rs — hutch wraps your file in a generated crate, so the
// SDK is `crate::electrobun` and your entry is `pub fn main()`. Full setup
// in the Rust hello world.
use crate::electrobun::{Core, Rect, WebviewOptions, WindowOptions};
...
```go
// src/go/main.go — import "electrobun" (hutch builds in GOPATH mode; no
// go.mod needed). Windows are created from a goroutine while
...
`electrobun/main` so 1.x applications can migrate incrementally.
