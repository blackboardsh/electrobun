...
| `buildEnvironment` | `"dev" \| "canary" \| "production" \| "stable" \| undefined` | Environment recorded in `build.json`; Hutch accepts `stable` as an alias for `production`, while the low-level type retains it for Electrobun 1.x bundle compatibility. |
