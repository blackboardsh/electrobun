...
development. It contains application, build, runtime, signing, and distribution
settings; product selection belongs in `hutch.config.ts`.
...
`hutch.config.ts` requires an exact `electrobun.version` such as `2.0.0` or
`2.1.0-beta.3` for an Electrobun project:

```typescript docs=syntax
export default {
  electrobun: {
    version: "2.0.0",
  },
...
That product version is independent of the thin npm bootstrap version and the
application version in `app.version`.

Hutch resolves and verifies that release under
`~/.dash/products/electrobun`, then projects its config types and language SDKs
into `.hutch/devkit`. Run `hutch electrobun sync` to refresh that projection
without building. `init`, `build`, `run`, and `dev` sync implicitly.
