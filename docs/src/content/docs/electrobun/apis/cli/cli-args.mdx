...
After extracting a template, `init` resolves the npm-paired Electrobun default
when supplied, otherwise the selected catalog release, for its initial
core/devkit sync. It also prepares any required native toolchain. If the
template defines an `install` task in
...
performs an initial sync to its resolved npm-paired or template-catalog release.
